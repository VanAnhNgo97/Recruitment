                ...
                if job['@type'] == 'JobPosting':
                    ...
                    if "jobBenefits" not in job:
                        ...
                        for element in benefits:
                            benefit = etree.tostring(element,method='html')
                            jobBenefits += benefit
                        job["jobBenefits"] = jobBenefits 
                    if "experienceRequirements" not in job:
                        requirements = description_dom.xpath("//*[contains(text(),'Quyền lợi')]/following-sibling::*[1]")
                        experienceRequirements = ""
                        for element in requirements:
                            requirement = etree.tostring(element,method='html')
